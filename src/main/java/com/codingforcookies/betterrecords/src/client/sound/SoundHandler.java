				...
				if(!soundList.containsKey(sounds.get(i).name)) {
					if(downloadSongs) {
						if(FileDownloader.isDownloading) {
							...
							nowPlaying = "Error: Please wait for song to finish downloading...";
		...
		new Thread(new Runnable() {
			public void run() {
				try {
					...
					nowPlaying = "Error: Could not read stream.";
					nowPlayingEnd = System.currentTimeMillis() + 5000;
				}
	...
	private static void playSourceDataLine(Sound snd, BlockPos pos, int dimension, BetterSoundType type, Object object) {
		try {
			...
			if(object instanceof File)
				in = convertToPCM(getAudioInputStream((File)object));
			else if(object instanceof BufferedInputStream)
				in = convertToPCM(getAudioInputStream((BufferedInputStream)object));
			else
				in = convertToPCM((AudioInputStream)object);
			...
			switch(type) {
				case SONG:
					nowPlaying = "Error: Could not read file.";
